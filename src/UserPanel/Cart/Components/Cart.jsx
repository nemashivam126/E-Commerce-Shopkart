...
const Cart = () => {
    ...
    const fetchCart = async () => {
        try {
            const response = await axios.get(`http://localhost:5000/shopkart/user/${user.id}/cart`, {
                headers: {
                    Token: token
                }
            });
    ...
    const updateCartQuantity = async (productId, quantity) => {
        try {
            const response = await axios.put(`http://localhost:5000/shopkart/user/${user.id}/updatecart`, {
                productId,
                quantity
            }, {
                headers: {
